...
describe('resolveMarkdownLink', () => {
  describe('non-markdown links', () => {
    ...
    it('returns API documentation links unchanged', () => {
      const result = resolveMarkdownLink(
        'api:SilverStripe\\Core\\Environment::getEnv()',
        '/current/file.md',
        '6',
      );
      expect(result).toBe('api:SilverStripe\\Core\\Environment::getEnv()');
    });

    it('returns simple API class links unchanged', () => {
      const result = resolveMarkdownLink(
        'api:SilverStripe\\ORM\\DataList',
        '/current/file.md',
        '6',
      );
      expect(result).toBe('api:SilverStripe\\ORM\\DataList');
    });

    it('returns API property links unchanged', () => {
      const result = resolveMarkdownLink(
        'api:SilverStripe\\CMS\\Model\\SiteTree->enforce_strict_hierarchy',
        '/current/file.md',
        '6',
      );
      expect(result).toBe('api:SilverStripe\\CMS\\Model\\SiteTree->enforce_strict_hierarchy');
    });
  });
