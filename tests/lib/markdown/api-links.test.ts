...
describe('API Links', () => {
  ...
  describe('rewriteAPILink', () => {
    ...
    it('should handle Environment::getEnv() correctly', () => {
      const result = rewriteAPILink('api:SilverStripe\\Core\\Environment::getEnv()');
      expect(result).toBe('https://api.silverstripe.org/search/lookup?q=SilverStripe%5CCore%5CEnvironment::getEnv()&version=6');
    });

    it('should handle Environment::getEnv() in version 5', () => {
      const result = rewriteAPILink('api:SilverStripe\\Core\\Environment::getEnv()', '5');
      expect(result).toBe('https://api.silverstripe.org/search/lookup?q=SilverStripe%5CCore%5CEnvironment::getEnv()&version=5');
    });

    it('should never emit relative links for API references', () => {
      // All API references should be absolute URLs to api.silverstripe.org
      const testCases = [
        'api:DataObject',
        'api:SilverStripe\\ORM\\DataList',
        'api:SilverStripe\\Core\\Environment::getEnv()',
        'api:SilverStripe\\CMS\\Model\\SiteTree->enforce_strict_hierarchy',
      ];
      testCases.forEach((apiRef) => {
        const result = rewriteAPILink(apiRef, '6');
        expect(result).toMatch(/^https:\/\/api\.silverstripe\.org/);
        expect(result).not.toMatch(/^(?!https:\/\/)/); // Ensure it's not relative
      });
    });
  });
