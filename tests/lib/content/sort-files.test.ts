...
describe('sort-files', () => {
  ...
  describe('sortDocuments', () => {
    ...
    it('sorts folders before files when order is the same', () => {
      const docs = [
        // Both have order 1, but folder should come first
        mockDoc('Usage', '/docs/linkfield/01_Usage.md', 1),
        {
          ...mockDoc('Basics', '/docs/linkfield/01_basics/index.md', 1),
          isIndex: true, // This is a folder
          fileAbsolutePath: '/docs/linkfield/01_basics/index.md',
        },
      ];

      const sorted = sortDocuments(docs);

      // Folder (isIndex=true) should come first
      expect(sorted[0].fileTitle).toBe('Basics');
      expect(sorted[0].isIndex).toBe(true);
      expect(sorted[0].order).toBe(1);

      // File (isIndex=false) should come second
      expect(sorted[1].fileTitle).toBe('Usage');
      expect(sorted[1].isIndex).toBe(false);
      expect(sorted[1].order).toBe(1);
    });

    it('maintains order priority with mixed folders and files', () => {
      const docs = [
        mockDoc('Usage', '/docs/linkfield/01_Usage.md', 1),
        {
          ...mockDoc('Configuration', '/docs/linkfield/02_configuration/index.md', 2),
          isIndex: true,
          fileAbsolutePath: '/docs/linkfield/02_configuration/index.md',
        },
        {
          ...mockDoc('Basics', '/docs/linkfield/01_basics/index.md', 1),
          isIndex: true,
        ...
        mockDoc('Advanced', '/docs/linkfield/03_Advanced.md', 3),
      ];

      const sorted = sortDocuments(docs);

      // Order 1: folder first
      expect(sorted[0].fileTitle).toBe('Basics');
      expect(sorted[0].isIndex).toBe(true);
      expect(sorted[0].order).toBe(1);
      ...
      // Order 2: folder
      expect(sorted[2].fileTitle).toBe('Configuration');
      expect(sorted[2].isIndex).toBe(true);
      expect(sorted[2].order).toBe(2);

      // Order 3: file
      expect(sorted[3].fileTitle).toBe('Advanced');
      expect(sorted[3].isIndex).toBe(false);
