...
describe('replaceChildrenMarkers', () => {
  ...
  describe('backtick handling (in HTML)', () => {
    it('skips replacement when marker is inside code tags', () => {
      const parent = createDoc('/en/6', 'Home', true, '/');
      const child = createDoc('/en/6/page1', 'Page', false, '/en/6', 'A page');

      const html = '<p>Use <code>[CHILDREN]</code> syntax in your documentation</p>';
      const allDocs = [parent, child];

      const result = replaceChildrenMarkers(html, parent, allDocs);

      expect(result).toContain('<code>[CHILDREN]</code>');
      expect(result).not.toContain('docs-overview');
      expect(result).not.toContain('docs-list');
    });

    it('skips markers with complex syntax inside code tags', () => {
      const parent = createDoc('/en/6', 'Home', true, '/');
      const child = createDoc('/en/6/page1', 'Page', false, '/en/6', 'A page');
      ...
      expect(result).toContain('<code>[CHILDREN Exclude="admin" asList]</code>');
      expect(result).not.toContain('docs-list');
    });
  });
});
